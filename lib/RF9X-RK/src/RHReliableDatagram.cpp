...
bool RHReliableDatagram::sendtoWait(uint8_t* buf, uint8_t len, uint8_t address)
{
	...
	Log.info("RHReliableDatagram wait-ack: from=%u to=%u id=%u retry=%u timeoutBase=%u len=%u",
	    _thisAddress,
	    address,
	    thisSequenceNumber,
	    retries - 1,
        ...
        while ((timeLeft = timeout - (millis() - thisSendTime)) > 0)
	{
		...
		{
		    #if FIELD_DEBUG_BUILD
		    Log.info("RHReliableDatagram wait-ack saw packet: from=%u to=%u id=%u flags=0x%02x awaitingFrom=%u awaitingId=%u",
			from,
			to,
			id,
			flags,
		    ...
		    {
			#if FIELD_DEBUG_BUILD
			Log.info("RHReliableDatagram wait-ack matched ACK: from=%u to=%u id=%u", from, to, id);
			#endif
			// Its the ACK we are waiting for
			return true;
		    }
		    else if (!(flags & RH_FLAGS_ACK) && to == _thisAddress)
		    {
			#if FIELD_DEBUG_BUILD
			Log.info("RHReliableDatagram wait-ack consumed non-ACK payload: from=%u to=%u id=%u flags=0x%02x while awaiting ACK from=%u id=%u",
			    from,
			    to,
			    id,
			    flags,
		    ...
		    {
			#if FIELD_DEBUG_BUILD
			Log.info("RHReliableDatagram wait-ack re-ack duplicate: from=%u to=%u id=%u", from, to, id);
			#endif
...
{  
    ...
    {
	#if FIELD_DEBUG_BUILD
	Log.info("RHReliableDatagram recv: from=%u to=%u id=%u flags=0x%02x ack=%u",
	    _from,
	    _to,
	    _id,
	    _flags,
	    (_flags & RH_FLAGS_ACK) ? 1 : 0);
	#endif
	// Never ACK an ACK
	if (!(_flags & RH_FLAGS_ACK))
	{
	    ...
	    {
		#if FIELD_DEBUG_BUILD
		Log.info("RHReliableDatagram recv ACKing payload: from=%u to=%u id=%u", _from, _to, _id);
		#endif
...
void RHReliableDatagram::acknowledge(uint8_t id, uint8_t from)
{
    ...
    uint8_t ack = '!';
	#if FIELD_DEBUG_BUILD
	Log.info("RHReliableDatagram ACK send: from=%u to=%u id=%u", _thisAddress, from, id);
	#endif
